...
document.getElementById('prev').addEventListener('click', () => {
    ...
    currentIndex = (currentIndex - 1 + items.length) % items.length; // 循环到最后一条
    items[currentIndex].classList.add('active');
});
...
document.getElementById('next').addEventListener('click', () => {
    items[currentIndex].classList.remove('active');
    currentIndex = (currentIndex + 1) % items.length; // 循环到第一条
    items[currentIndex].classList.add('active');
});
docReady(navbarInit);
            ...
            fetch(`/get_style/${styleId}`)
                ...
                .then(data => {
                    if (data.error) {
                        ...
                        // 更新页面上的内容
                        document.querySelector("p.language_style").textContent = data.language_style;
                        document.querySelector("p.class_style").textContent = data.class_style;
                        document.querySelector("p.file_name").textContent = data.file_name;
                        document.querySelector("p.outline").textContent = data.outline;
